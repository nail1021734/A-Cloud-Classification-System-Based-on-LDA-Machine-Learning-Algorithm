...
for i in filepath:
	...
	if ftype==".txt" or ftype==".pdf" or ftype==".docx":
		...
		filterfilePath.append(i)
texts=[]
count=0
for i in filterfilePath:
	texts.append([])
	filename=i.split("/")[-1]
	ftype=os.path.splitext(i)[-1]
	s=""
...
model = cv.fit(df)
model.write().overwrite().save("hdfs:/User/"+user+"/"+user+"_cv.model")
...
# 	print("#######################################################")
# 	print(x)
lda = LDA(k=clusternum,maxIter=300)
#lda.save("hdfs:/"+user+"_text.model")
# print("ss")
ldamodel =lda.fit(result)
ldamodel.write().overwrite().save("hdfs:/User/"+user+"/"+user+"_lda.model")
...
for i in topics.select('topicDistribution').collect():
	print(filterfilePath[count].split("/")[-1],end=' ')
